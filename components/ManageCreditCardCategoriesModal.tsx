...
export default function ManageCreditCardCategoriesModal({
  categories, onAddCategory, onDeleteCategory, onAddSubcategory, onDeleteSubcategory, onClose,
}: Props) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="flex w-full max-w-sm flex-col rounded-xl bg-white shadow-xl dark:bg-zinc-900" style={{ maxHeight: "85vh" }}>
        <div className="flex items-center justify-between border-b border-zinc-100 px-5 py-4 dark:border-zinc-800">
          <h2 className="text-base font-semibold text-zinc-900 dark:text-zinc-50">Categorias do Cartão</h2>
          <button
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          {showAddCategory && (
            <div className="flex gap-2">
              ...
              />
              <button onClick={handleAddCategory} className="rounded-lg bg-zinc-900 px-3 py-2 text-xs font-semibold text-white hover:bg-zinc-700 dark:bg-zinc-50 dark:text-zinc-900">Salvar</button>
              <button onClick={() => setShowAddCategory(false)} className="rounded-lg border border-zinc-200 px-3 py-2 text-xs text-zinc-500 hover:border-zinc-400 dark:border-zinc-700">×</button>
            </div>
          )}
            ...
            <div key={cat.id} className="rounded-lg border border-zinc-100 bg-zinc-50 p-3 dark:border-zinc-800 dark:bg-zinc-800/50">
              ...
              {addingSubFor === cat.id ? (
                <div className="mt-2 flex gap-2">
                  ...
                  />
                  <button onClick={() => handleAddSubcategory(cat.id)} className="rounded-lg bg-zinc-900 px-2.5 py-1.5 text-xs font-semibold text-white hover:bg-zinc-700 dark:bg-zinc-50 dark:text-zinc-900">OK</button>
                  <button onClick={() => setAddingSubFor(null)} className="rounded-lg border border-zinc-200 px-2 py-1 text-xs text-zinc-500 hover:border-zinc-400 dark:border-zinc-700">×</button>
                </div>
              ) : (
                <button
